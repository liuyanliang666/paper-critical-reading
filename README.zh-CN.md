...
在 Codex 或 Claude 的聊天中阅读、追问论文，并附上**可以点击定位的原文引用**。点击引用后，本地阅读器打开保存的 PDF，滚动到原句，并逐行画框、高亮。继续追问时复用同一篇论文，无需每次重新生成报告。

原有的第一性原理批判性精读框架仍然保留。现在可以按具体问题回答，也可以按要求输出完整精读。
...
工具导入一次论文，随后技能检索并阅读上下文，为准确原句创建引用。你继续提问时，复用同一 `paper_id`，只新增这次回答需要的引用。想要原来的完整分析时，可以直接要求“对这篇论文做完整批判性精读”。
...
完整精读仍包含 TL;DR、Task、Challenge、Method、Insight & Novelty、Evidence & Validation、Potential Flaw、Motivation；未明确陈述的灵感和动机继续标为推测。多条重要判断现在可以分别附上原文证据，并遵守宿主的适用引用限制。
